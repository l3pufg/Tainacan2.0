        ...
        //if ($property['metas']['socialdb_property_object_is_facet'] == 'false'):
            ...
            <div class="form-group">
                <label for="object_tags"><?php echo $property['name']; ?></label>
                    ...
                    <button type="button" onclick="save_object_property('<?php echo $property['id']; ?>', '<?php echo $object_id; ?>')" id="single_save_<?php echo $property['id']; ?>_<?php echo $object_id; ?>"class="btn btn-default btn-xs" style="display: none;"><span class="glyphicon glyphicon-floppy-disk"></span></button> <a target="_blank" class="btn btn-primary btn-xs" href="<?php echo get_permalink($property['metas']['collection_data'][0]->ID); ?>"><?php _e('Add new','tainacan'); ?><?php echo ' ' . $property['metas']['collection_data'][0]->post_title; ?></a>
                    <input type="text" onkeyup="autocomplete_object_property('<?php echo $property['id']; ?>', '<?php echo $object_id; ?>');" id="single_autocomplete_value_<?php echo $property['id']; ?>_<?php echo $object_id; ?>" placeholder="<?php _e('Type the three first letters of the object ','tainacan'); ?>"  class="chosen-selected form-control" disabled="disabled" />
                    <select onclick="clear_select_object_property(this);"  id="single_property_value_<?php echo $property['id']; ?>_<?php echo $object_id; ?>" multiple class="chosen-selected2 form-control" style="height: auto;" multiple name="category_moderators[]" id="chosen-selected2-user" disabled="disabled" >
    ...
    foreach ($property_data as $property) {
        ...
        <div class="form-group">
            ...
            <?php if ($property['type'] == 'text') { ?>     
                <input disabled="disabled" value="<?php if($property['metas']['value']) echo $property['metas']['value'][0]; ?>" type="text" id="single_property_value_<?php echo $property['id']; ?>_<?php echo $object_id; ?>" class="form-control" name="socialdb_property_<?php echo $property['id']; ?>" <?php if (!$property['metas']['socialdb_property_required']): echo 'required="required"';
            endif; ?>>
        <?php }elseif ($property['type'] == 'textarea') { ?>   
                <textarea disabled="disabled" id="single_property_value_<?php echo $property['id']; ?>_<?php echo $object_id; ?>" class="form-control" name="socialdb_property_<?php echo $property['id']; ?>" <?php if (!$property['metas']['socialdb_property_required']): echo 'required="required"';
            endif; ?>><?php if($property['metas']['value']) echo $property['metas']['value'][0]; ?></textarea>
        <?php }else { ?> 
                <input disabled="disabled" value="<?php if($property['metas']['value']) echo $property['metas']['value'][0]; ?>" id="single_property_value_<?php echo $property['id']; ?>_<?php echo $object_id; ?>" type="text" class="form-control input_date" name="socialdb_property_<?php echo $property['id']; ?>" <?php if (!$property['metas']['socialdb_property_required']): echo 'required="required"';
            endif; ?>>
        <?php } ?> 
            <input type="hidden" id="single_property_<?php echo $property['id']; ?>_<?php echo $object_id; ?>_value_before" name="property_<?php echo $property['id']; ?>_<?php echo $object_id; ?>_value_before" value="<?php if (is_array($property['metas']['value'])) echo implode(',', $property['metas']['value']); ?>">
        </div>              
    <?php } ?>
<?php endif; 
