...
class CategoryModel extends Model {
    ...
    public function add($data) {
        ...
        if (!$is_new) {
            if ($data['category_parent_id'] == '0'||
                    ...
                    ||$data['category_parent_id'] == 'shared_categories') {// se nao o usuario nao setou o parent
        ...
        if (count($children) > 0) {
            foreach ($children as $child) {
                ...
                if (count($children_of_child) > 0 || (!empty($children_of_child) && $children_of_child)) {// se tiver descendentes
                   $dynatree['children'][] = array('title' => $child->name,'hideCheckbox' => $hide_checkbox, 'key' => $child->term_id, 'isLazy' => true,'addClass' => $classCss);
                } else {// se nao tiver filhos
                    $dynatree['children'][] = array('title' => $child->name,'hideCheckbox' => $hide_checkbox, 'key' => $child->term_id,'addClass' => $classCss);
                }
                $counter++;
                if ($counter == 9) {
                    $dynatree['children'][] = array('title' => __('See more','tainacan'), 'hideCheckbox' => true, 'key' => $all_data['metas']['socialdb_property_term_root'] . '_moreoptions', 'isLazy' => true, 'addClass' => 'more');
                    break;
                }
            }
        }
    ...
    public function generate_user_categories_dynatree($data, $dynatree,$hide_checkbox = false) {
        $classCss = 'user_img';
    ...
    public function initPropertyCategoriesDynatree($data) {
        ...
        foreach ($facets_id as &$facet_id) {
            $facet = get_term_by('id', $facet_id->term_id, 'socialdb_category_type');
            if ($facet) {
                $dynatree[] = array('title' => ucfirst($facet->name), 'key' => $facet->term_id, 'isLazy' => true, 'data' => $url, 'isFolder' => true, 'expand' => true, 'hideCheckbox' => true, 'addClass' => $classCss);
                $dynatree[end(array_keys($dynatree))] = $this->getPropertyChildrenDynatree($facet->term_id, $dynatree[end(array_keys($dynatree))], $data['collection_id']);
            }
        }
        return json_encode($dynatree);
    }
            ...
            foreach ($children as $child) {
                ...
                if (count($children_of_child) > 0 || (!empty($children_of_child) && $children_of_child)) {// se tiver descendentes
                    if (in_array($child->term_id, CollectionModel::get_facets($collection_id))) { // verifico se e uma faceta
                        $dynatree['children'][] = array('title' => $child->name, 'key' => $child->term_id, 'isLazy' => true, 'select' => true);
                    } else {// se nao
                        $dynatree['children'][] = array('title' => $child->name, 'key' => $child->term_id, 'isLazy' => true);
                    }
                } else {// se nao tiver filhos
                    if (in_array($child->term_id, CollectionModel::get_facets($collection_id))) {// se for faceta
                        $dynatree['children'][] = array('title' => $child->name, 'key' => $child->term_id, 'select' => true);
                    } else {
                        $dynatree['children'][] = array('title' => $child->name, 'key' => $child->term_id);
                    }
                }
            }
        }
    ...
    public function getPropertyChildrenDynatree($facet_id, $dynatree, $collection_id) {
        $children = $this->getChildren($facet_id);
        if (count($children) > 0) {
            foreach ($children as $child) {
                $children_of_child = $this->getChildren($child->term_id);
                if (count($children_of_child) > 0 || (!empty($children_of_child) && $children_of_child)) {// se tiver descendentes
                    $dynatree['children'][] = array('title' => $child->name, 'key' => $child->term_id, 'hideCheckbox' => true, 'isLazy' => true);
                } else {// se nao tiver filhos
                    $dynatree['children'][] = array('title' => $child->name, 'key' => $child->term_id, 'hideCheckbox' => true);
                }
            }
        }
        return $dynatree;
        ...
        if (is_array($childrens) && count($childrens) > 0) {
            foreach ($childrens as $child) {
                ...
                if (is_array($sub_childrens) && count($sub_childrens) > 0) {
                    if (in_array($child->term_id, CollectionModel::get_facets($data['collection_id']))) {
    ...
    public function get_categories_by_owner($owner_id, $parent = 0) {
        ...
        $query = "
			...
			INNER JOIN $wp_term_taxonomy tt ON t.term_id = tt.term_id
                        INNER JOIN $wp_taxonomymeta tx on tx.taxonomy_id = tt.term_id
    ...
    public function get_categories_shared_by_owner($owner_id) {
        ...
        $query = "
                    SELECT * FROM $wp_terms t
                    INNER JOIN $wp_term_taxonomy tt ON t.term_id = tt.term_id 
                    INNER JOIN $wp_taxonomymeta tx ON t.term_id = tx.taxonomy_id
    ...
    public function get_metas($data) {
        ...
        $query = "
			SELECT * FROM $wp_taxonomymeta 
				WHERE taxonomy_id = {$data['category_id']}
		";
    ...
    public function get_properties($collection_id, $categories) {
        $all_properties_id = [];
        $all_properties_id = $this->get_parent_properties($this->get_collection_category_root($collection_id), $all_properties_id);
        if (!empty($categories)) {
            foreach ($categories as $category) {
                if(is_object($category)):
                    $all_properties_id = $this->get_parent_properties($category->term_id, $all_properties_id);
                endif;
            }
         }
    ...
    /* function get_collection_properties($collection_id,$all_properties_id) */
    /* @param int  $collection_id O id da colecao
      /* @param array $categories O array com ids das propriedades que ja foram encontradas ate o meomento
      /* @return array  Os ids pas propriedades encontradas apenas na colecao */
    /* @author: Eduardo */

    public function get_collection_properties($collection_id, $all_properties_id) {
        $category_root = $this->get_collection_category_root($collection_id);
        $all_properties_id = $this->get_parent_properties($category_root, $all_properties_id);
        return $all_properties_id;
    }
    ...
    * @return array  Gera os arquivos xml dentro da pasta a ser zipada para exportacao
    ...
    public function export_zip_taxonomies($terms_id){
        if(!empty($terms_id)){
            foreach ($terms_id as $term_id) {
                ob_clean();
                $df = fopen(dirname(__FILE__).'/../export/package/taxonomies/'.$term_id.'.xml', 'w');
    ...
    public function verify_name_in_taxonomy($data) {
        ...
        $result = [];
        $is_new = $this->verify_category(['category_parent_id'=>$data['parent_id'],'category_name'=>$data['suggested_name']]);
        if ($is_new) {
