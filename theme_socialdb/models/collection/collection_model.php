...
class CollectionModel extends Model {
    ...
    public function simple_add($data) {
        ...
        create_root_collection_category($post->ID, __('Categories of ','tainacan') . $data['collection_object'] . __(' of ','tainacan') . $data['collection_name']); //(Criada em: functions.php) cria a categoria inicial que identifica os objetos da colecao
        return $post->ID;
    }
    ...
    public function update($data) {
        ...
        if ($data['socialdb_collection_parent'] != '' && ($data['socialdb_collection_parent'] != get_post_meta($post_id, 'socialdb_collection_parent', true))) {
            ...
            if ($data['socialdb_collection_parent'] == 'collection_root') {
                ...
                $category_root_id = $this->get_category_root($data['collection_id']);
    ...
    public function extend_collection($collection_id, $parent_collection_root_id) {
        ...
        $collection_properties = get_term_meta($this->get_category_root($collection_id), 'socialdb_category_property_id');
        if (!$collection_properties) {
            delete_term_meta($this->get_category_root($collection_id), 'socialdb_category_property_id');
        }
        if ($properties_to_add && is_array($collection_properties)) {
            foreach ($properties_to_add as $property_to_add) {
                if (!in_array($property_to_add, $collection_properties)) {
                    $this->vinculate_property($this->get_category_root($collection_id), $property_to_add);
                    ...
                    $this->vinculate_objects_with_property($property_to_add, $collection_id, $this->get_category_root($collection_id));
                }
            }
        }
    }
        ...
        if ($properties_to_remove && is_array($properties_to_remove)) {
            foreach ($properties_to_remove as $property_to_remove) {
                ...
                if (!$this->is_repository_property($category_property_id)&& in_array($property_to_remove, $collection_properties) // se nao for propriedade do repositorio, se estiver no array de propriedades do pai anterior e a categoria root de onde foi criada nao for a atual
                        && $category_property_id != $this->get_category_root($collection_id)) {
                    delete_term_meta($this->get_category_root($collection_id), 'socialdb_category_property_id', $property_to_remove);
                }
            }
        }
    }
    ...
    public static function get_facets($collection_id) {
        $facets = array_filter(array_unique(get_post_meta($collection_id, 'socialdb_collection_facets')));
        return $facets;
    }
...
//            ";
    ...
    public function list_ordenation($data) {
        ...
        $category_root = $this->get_category_root($data['collection_id']);
        //$all_properties_id = get_term_meta($category_root, 'socialdb_category_property_id');
        $all_properties_id = $this->get_parent_properties($category_root, []);
    ...
    public function get_properties_autocomplete($collection_id, &$array_autocomplete) {
        ...
        $root_category = $this->get_category_root($collection_id);
    ...
    /**
     * funcao que busca os autores mais participativos de uma colecao
     * @param int $collection_id O id da colecao que esa sendo montado o autocomplete
     * @author: Eduardo Humberto 
     */
    ...
    public function get_order_category_properties($data) {
        ...
        foreach ($filters as $filter) {
            if (strpos($filter, '_') === false && $filter != $this->get_category_root($data['collection_id'])) {
