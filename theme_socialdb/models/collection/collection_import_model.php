...
class CollectionImportModel extends CollectionModel {
    ...
    public function add_thumbnail_collection($dir_created,$collection_id) {
        if(is_file($dir_created.'/package/metadata/thumbnail.png')){
            $thumbnail_id = $this->insert_attachment_file($dir_created.'/package/metadata/thumbnail.png', $collection_id);
            set_post_thumbnail($collection_id, $thumbnail_id);
        }elseif(is_file($dir_created.'/package/metadata/thumbnail.jpg')){
            $thumbnail_id = $this->insert_attachment_file($dir_created.'/package/metadata/thumbnail.jpg', $collection_id);
             set_post_thumbnail($collection_id, $thumbnail_id);
        }elseif(is_file($dir_created.'/package/metadata/thumbnail.gif')){
            $thumbnail_id = $this->insert_attachment_file($dir_created.'/package/metadata/thumbnail.gif', $collection_id);
            set_post_thumbnail($collection_id, $thumbnail_id);
        }elseif(is_file($dir_created.'/package/metadata/thumbnail.jpeg')){
            $thumbnail_id = $this->insert_attachment_file($dir_created.'/package/metadata/thumbnail.jpeg', $collection_id);
            set_post_thumbnail($collection_id, $thumbnail_id);
        }
    }
    /**
    ...
    public function add_cover_collection($dir_created,$collection_id) {
        if(is_file($dir_created.'/package/metadata/cover.png')){
            $cover_id = $this->insert_attachment_file($dir_created.'/package/metadata/cover.png', $collection_id);
            ...
            $cover_id = $this->insert_attachment_file($dir_created.'/package/metadata/cover.jpg', $collection_id);
              update_post_meta($collection_id, 'socialdb_collection_cover_id', $cover_id);
        }elseif(is_file($dir_created.'/package/metadata/cover.gif')){
            $cover_id = $this->insert_attachment_file($dir_created.'/package/metadata/cover.gif', $collection_id);
            update_post_meta($collection_id, 'socialdb_collection_cover_id', $cover_id);
        }elseif(is_file($dir_created.'/package/metadata/cover.jpeg')){
            $cover_id = $this->insert_attachment_file($dir_created.'/package/metadata/cover.jpeg', $collection_id);
             update_post_meta($collection_id, 'socialdb_collection_cover_id', $cover_id);
        }
    }
    ############################# IMPORTAR TAXONOMIAS ############################################
   ...
   public function insert_properties($xml,$category_root_id){
       ...
       if(!is_array($properties)): $properties[]=$properties; endif;
       if($properties){
           foreach ($properties as $property) {
               if(isset($property->socialdb_property_term_root)){
                   $property_id = $this->add_property_term($property,$property->socialdb_property_created_category);
                   ...
                   $property_id = $this->add_property_data($property,$property->socialdb_property_created_category);
                    add_term_meta($category_root_id, 'socialdb_category_property_id',$property_id);
                    $properties_id[] = $property_id;
               }elseif(isset($property->socialdb_property_object_category_id)){
                   $property_id = $this->add_property_object($property,$property->socialdb_property_created_category);
                    add_term_meta($category_root_id, 'socialdb_category_property_id',$property_id);
                    $properties_id[] = $property_id;
               }elseif(isset($property->ranking_type)){
                   $property_id = $this->add_property_ranking($property,$property->socialdb_property_created_category);
                   add_term_meta($category_root_id, 'socialdb_category_property_id',$property_id);
                   $properties_id[] = $property_id;
               }
           }
     ...
     * @author: Eduardo Humberto 
     */
   public function add_property_term($property,$socialdb_collection_object_type) {
   ...
   /**
     ...
     * @author: Eduardo Humberto 
     */
   public function add_property_data($property,$socialdb_collection_object_type) {
   ...
   /**
     ...
     * @author: Eduardo Humberto 
     */
   public function add_property_object($property,$socialdb_collection_object_type) {
   ...
   /**
     ...
     * @author: Eduardo Humberto 
     */
   public function add_property_ranking($property) {
   ...
   public function add_facets($xml,$collection_id) {
        $facets = $xml->facets->facet;
       if($facets&&!is_array($facets)): $facets[]=$facets; endif;
       if($facets){
   ...
   public function add_channels($xml,$collection_id) {
        $channels = $xml->channels->channel;
       if($channels&&!is_array($channels)): $channels[]=$channels; endif;
       if($channels){
