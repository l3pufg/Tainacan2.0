...
class RankingModel extends Model {
    ...
    public function add($data) {
        ...
        if (!is_wp_error($new_ranking) && $new_ranking['term_id']) {//
            $result[] = update_term_meta($new_ranking['term_id'], 'socialdb_property_created_category', $this->get_category_root($data['collection_id']));
            instantiate_metas($new_ranking['term_id'], $type->name, 'socialdb_property_type', true);
            $result[] = $this->vinculate_property($this->get_category_root($data['collection_id']), $new_ranking['term_id']);
            //possivelmente um problema
            $this->vinculate_objects_with_property($new_ranking['term_id'], $data['collection_id'], $this->get_category_root($data['collection_id']));
            $this->insert_properties_hierarchy($this->get_category_root($data['collection_id']), $new_ranking['term_id']);
            $data['success'] = 'true';
        } else {
            $data['success'] = 'false';
    ...
    public function create_santard_vote($data) {
        ...
        $category_parent_root_id = get_term_by('id', $this->get_category_root($data['collection_id']), 'socialdb_category_type')->parent ;
     ...
     * @autor: Eduardo 
     */
    public function save_vote($data, $update_value = true) {
    ...
    public function calculate_vote_stars($property_id, $object_id) {
        ...
        if ($result && is_array($result) && count($result) > 0) {
            ...
            $values['final_score'] = round(($sum / count($result)) / 2, 2);
        } else {
            $values['count'] = 0;
