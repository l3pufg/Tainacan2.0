...
class ZipModel extends Model {
    ...
    public function get_collection_images($collection_id) {
        ...
        if($thumbnail_id){
          ...
          copy($fullsize_path, dirname(__FILE__).'/package/metadata/thumbnail.'.$ext);
        }
        if(get_post_meta($collection_id, 'socialdb_collection_cover_id',true)){
          ...
          $ext = pathinfo($fullsize_path, PATHINFO_EXTENSION);
          copy($fullsize_path, dirname(__FILE__).'/package/metadata/cover.'.$ext);
        }
    }
    /**
     * @signature - export_collection($collection_id)
     ...
     * @author: Eduardo 
     */
    public function generate_taxonomy_files($collection_id) {
        ...
        $categoryModel->export_zip_taxonomies(array_unique($terms_id));
        $xml_collection = $this->export_collection_settings($collection_id, $all_properties_id);
        $this->create_xml_file(dirname(__FILE__).'/package/metadata/administrative_settings.xml', $xml_collection);
    }
    ...
    function export_collection_settings($collection_id,$properties_id) {
        ...
        $xml .= '<collection>';
            $xml .= '<post_title>'.$collection_post->post_title.'</post_title>';
            $xml .= '<post_content>'.$collection_post->post_content.'</post_content>';
     ...
     * @author: Eduardo 
     */  
     public function export_items($collection_id) {
        $items = $this->get_collection_posts($collection_id);
        if($items&&is_array($items)){
            foreach ($items as $index => $item) {
                mkdir(dirname(__FILE__).'/package/items/'.$index);
                $this->generate_item_xml($item, $index);
                $this->export_item_thumbnail($item->ID, $index);
                $this->export_files($item->ID, $index);
                $this->export_content($item->ID, $index);
            }
        }
     }
     
    public function generate_item_xml($item,$index) {
        ...
        $xml .= "<socialdb_object_dc_source>".get_post_meta( $item->ID, 'socialdb_object_dc_source', TRUE)."</socialdb_object_dc_source>";
        ...
        $xml .= '</properties>';
        $xml .= '</item>';
        $this->create_xml_file(dirname(__FILE__).'/package/items/'.$index.'/item.xml', $xml);
    }
     /**
     ...
     */  
     ...
     public function generate_properties_item($item_id,$xml) {
         ...
         if(!empty($properties)){
             foreach ($properties as $id => $values) {
                 $xml .= '<property>';
                 $xml .= '<id>'.$id.'</id>';
                 $xml .= '<value>'.  implode('|', $values).'</value>';
                 $xml .= '</property>';
             }
         }
         return $xml;
      ...
      * 
      */
     public function export_files($item_id,$index) {
      ...
      * 
      */
     public function export_item_thumbnail($item_id,$index) {
         $thumbnail_id = get_post_thumbnail_id($item_id);
        if($thumbnail_id){
          $fullsize_path = get_attached_file( $thumbnail_id ); // Full path
          $ext = pathinfo($fullsize_path, PATHINFO_EXTENSION);
          $filename_only = basename($fullsize_path); // Just the file name
          copy($fullsize_path, dirname(__FILE__).'/package/items/'.$index.'/thumbnail.'.$ext);
        }
     }
     /**
      * 
      */
     public function export_content($item_id,$index) {
        $object_content = get_post_meta($item_id,'socialdb_object_content',true);
