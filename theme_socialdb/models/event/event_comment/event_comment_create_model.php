...
class EventCommentCreate extends EventModel {
    ...
    public function add_comment($event_id,$data,$automatically_verified) {
        ...
        if($user_id&&$user_id!=0){
           $result = $commentModel->add($object_id, $comment_content, $parent_id, '', '', '', $user_id);
           ...
           $result = $commentModel->add($object_id, $comment_content, $parent_id, $comment_author_name, $comment_author_email, $comment_author_website, $user_id);
           $commentarr['comment_ID'] = $result['comment_id'];
           $commentarr['comment_approved'] = 1;
           wp_update_comment( $commentarr );
        }
