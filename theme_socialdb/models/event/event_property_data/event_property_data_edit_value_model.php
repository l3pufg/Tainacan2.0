...
class EventPropertyDataEditValue extends EventModel {
    ...
    public function generate_title($data) {
        ...
        if($data['socialdb_event_property_data_edit_value_attribute_value']!=''){
            ...
            $title = _('Delete all values of the data property ','tainacan').'<b>'.$property->name.'</b>'
                . __(' in the the object ','tainacan') . $object->post_title;
        }
        return $title;
    }
    ...
    public function update_property_value($event_id,$data,$automatically_verified) {
        ...
        $meta = get_post_meta($object_id,'socialdb_property_'.$property,true);
        if($meta||$meta===''){
           $result = update_post_meta($object_id,'socialdb_property_'.$property, $value);
        }else{
           $result =add_post_meta($object_id, 'socialdb_property_'.$property, $value);
        }
