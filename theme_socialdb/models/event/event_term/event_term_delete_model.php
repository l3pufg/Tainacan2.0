...
class EventTermDelete extends EventModel {
    ...
    public function generate_title($data) {
        ...
        $title = __('Delete the category ','tainacan') . '(' . $category->name . ')' . __(' in the collection ','tainacan') . '<b>' . $collection->post_title . '</b>';
        return $title;
    }
    ...
    public function delete_category($event_id, $data, $automatically_verified) {
        ...
        if ($categoryModel->is_facet($data['category_id'], $data['collection_id'])) {
