...
class ObjectModel extends Model {
    ...
    public function add($data) {
        ...
        $category_root_id = $this->collection_model->get_category_root($data['collection_id']);
        $user_id = get_current_user_id();
        if ($user_id == 0) {
            $user_id = get_option('anonimous_user');
        }
    ...
    public function validate_form($data) {
        ...
        if ($data['object_type'] != 'text' && $data['object_from'] == 'internal') {
            ...
            if ($data['object_type'] == 'video' && !in_array($ext, ['mp4', 'm4v', 'wmv', 'avi', 'mpg', 'ogv', '3gp', '3g2'])) {
                ...
                return $data;
            } elseif ($data['object_type'] == 'audio' && !in_array($ext, ['mp3', 'm4a', 'ogg', 'wav'])) {
                $data['validation_error'] = true;
                $data['title'] = __('Error', 'tainacan');
    ...
    public function update_classifications($classification_string, $object_id, $collection_id = null) {
        ...
        if (!empty($categories)) {
            ...
            $categories[] = (int) $this->collection_model->get_category_root($collection_id);
            ...
            $category_root_id = $this->collection_model->get_category_root($collection_id);
            //categoria raiz da colecao
                ...
                if (!is_array($data["socialdb_property_$property_id"]) && $data["socialdb_property_$property_id"] !== '') {
                    ...
                    foreach ($data["socialdb_property_$property_id"] as $value) {
                        ...
                        if (isset($dados->metas) && ($dados->metas->socialdb_property_object_is_reverse == 'true')) {
                            update_post_meta($value, "socialdb_property_" . $dados->metas->socialdb_property_object_reverse, $object_id);
                        }
                    }
                } else {
                    ...
                    if (isset($dados->metas) && ($dados->metas->socialdb_property_object_is_reverse == 'true' && $property_reverse)) {
                        update_post_meta($property_reverse, "socialdb_property_" . $dados->metas->socialdb_property_object_reverse, '');
                    }
    ...
    public function insert_properties_terms($data, $object_id) {
        if ($data['properties_id'] !== '') {
            $properties_id = explode(',', $data['properties_id']);
            foreach ($properties_id as $property_id) {
                if (!is_array($data["socialdb_propertyterm_$property_id"]) && $data["socialdb_propertyterm_$property_id"] !== '') {
    ...
    public function get_args($data) {
        $args['category_root_id'] = $this->collection_model->get_category_root($data['collection_id']);
    ...
    public function list_object($args = null) {
        ...
        $tax_query[] = array(
            ...
            'field' => 'id',
            'terms' => array($this->collection_model->get_category_root($args['collection_id']))
        );
        //tipo de ordenacao
    ...
    public function set_type_order($data) {
        if ($data['order_by'] == '' || !isset($data['order_by'])) {
    ...
    public function get_tax_query($categories, $collection_id, $tags) {
        ...
        $tax_query[] = array(
            'taxonomy' => 'socialdb_category_type',
            'field' => 'id',
            'terms' => $this->collection_model->get_category_root($collection_id),
            'operator' => 'IN'
        );
        if ($categories) {
    ...
    public function show_object_properties($data) {
        ...
        return $this->set_data_object_properties($all_properties, $data);
    }
    ...
    public function set_data_object_properties($properties, $data) {
        ...
        $data['category_root_id'] = $this->get_category_root($data['collection_id']);
        return $data;
    }
